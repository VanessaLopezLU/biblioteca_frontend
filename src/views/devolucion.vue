        ...
        <v-row class="justify-center">
          <v-col cols="8">
            <v-card width="500px" class="mx-auto mt-2">
              <v-card-title>DEVOLUCIÓN DE PRÉSTAMOS</v-card-title>
              <v-card-text>
                <v-form ref="form">
                  <v-row no-gutters>
  ...
  methods: {
    verDetalles(index) {
      if (index != null) {
        ...
        this.itemsDetalle = this.itemsPrestamo[index].detalle.map((detalle) => {
          ...
          this.estadosSeleccionados.push({
            id: detalle.equipo.estado_equipo.id,
            estado: detalle.equipo.estado_equipo.estado,
          });
          return {
            tipo_equipo: detalle.equipo.tipo_equipo.tipo,
            serial: detalle.equipo.serial,
            estado: detalle.equipo.estado_equipo.id,
          };
        });
    ...
    async devolucion() {
      if (this.validarEstados()) {
        this.$emit('loading', 'Devolviendo equipos, espere un momento...');
        const paquete = {
          idPrestamo: this.itemsPrestamo[this.indexSelecionado].id,
          equipos: this.itemsPrestamo[this.indexSelecionado].detalle.map(
            (detalle, index) => {
              return {
                id_equipo: detalle.equipo.serial,
                estado_equipo: this.estadosSeleccionados[index].id,
